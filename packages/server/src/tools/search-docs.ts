...
export async function handleSearchDocs(
  ...
  // Use hybrid search (TF-IDF + vector) when available, else TF-IDF only
  const rawResults = hybridSearch
    ? await hybridSearch.search(args.query, docs, 10)
    : searchEngine.search(args.query, docs, 10).map((r) => ({
        doc: r.doc,
        score: r.score,
        tfidfScore: r.score,
        vectorScore: 0,
  ...
  // Normalize to the format the rest of the function expects
  ...
  const searchMode = hybridSearch?.hasVectorSearch() ? "hybrid" : "keyword";
  ...
  let header = `Found ${results.length} results for "${args.query}" (${searchMode})`;
