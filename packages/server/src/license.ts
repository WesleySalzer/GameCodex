...
export function getLicenseKey(): string | null {
  const envKey = process.env.GAMECODEX_LICENSE;
...
export async function validateLicense(): Promise<LicenseResult> {
  ...
  if (!key) {
    return { tier: "free", message: "[gamecodex] Running in free tier" };
  }
  ...
  if (throttle.throttled) {
    ...
    return {
      tier: "free",
      message: `[gamecodex] License: too many failed attempts — retry in ${retrySeconds}s`,
    };
  }
