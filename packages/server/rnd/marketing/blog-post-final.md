...
GameCodex has **10 tools** — lean by design:
...
GameCodex has **10 tools with minimal schema overhead**. Section extraction means you pull exactly what you need. The philosophy: knowledge density per token, not tool count.
...
- **Cover image:** Terminal showing `npx gamecodex` with game dev icons
