...
Base URL: `https://api.gamecodex.com` (or `gamecodex-api.<account>.workers.dev` initially)
...
- [ ] Configure custom domain (api.gamecodex.com) in Cloudflare dashboard
...
├── doc-cache.ts                  # Disk cache with TTL (~/.gamecodex/cache/)
